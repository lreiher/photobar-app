...
@interface ViewControllerLayoutCustom3 ()
...
@implementation ViewControllerLayoutCustom3
...
-(void)loadView {
    ...
    [super loadView];
    self.pathComponent = @"layoutC_image";
}
...
-(void)setInitialValues {
    ...
    [self.sharedDefaults setBool:true forKey:@"layoutC_willAutoLayout"];
...
- (void)viewDidLayoutSubviews {
    if ([self.sharedDefaults boolForKey:@"layoutC_willAutoLayout"]) {
        [self didAutolayout];
    }
    if ([self.sharedDefaults integerForKey:@"layoutC_height"]) {
        self.heightSlider.value = [self.sharedDefaults integerForKey:@"layoutC_height"];
    }
...
-(void)didAutolayout {
    ...
    if ([self.sharedDefaults boolForKey:@"layoutC_isReady"]) {
        
        self.heightSlider.hidden = YES;
        
        self.cropSize = CGSizeMake([self.sharedDefaults floatForKey:@"layoutC_cropSizeW"], [self.sharedDefaults floatForKey:@"layoutC_cropSizeH"]);
        self.numberOfImages = [self.sharedDefaults integerForKey:@"layoutC_count"];
        ...
        [self.sharedDefaults setBool:(self.segmentedControl.selectedSegmentIndex == 0) forKey:@"layoutC_isHorizontal"];
        [self.sharedDefaults synchronize];
    }
    
    [self determinePlaceholderVisibility];
    
    [self.sharedDefaults setBool:false forKey:@"layoutC_willAutoLayout"];
    [self.sharedDefaults synchronize];
}
...
-(void)resetLayout {
    ...
    [self.sharedDefaults setBool:false forKey:@"layoutC_isReady"];
...
- (IBAction)confirmedSize:(id)sender {
    ...
    if (self.imageNoSlider.hidden && sender != nil) {    //confirmed size
        ...
        [self.sharedDefaults setInteger:self.heightSlider.value forKey:@"layoutC_height"];
        [self.sharedDefaults synchronize];
...
-(void)setAdditionalValues {
    ...
    [self.sharedDefaults setFloat:imgRatio forKey:@"layoutC_ratio"];
    [self.sharedDefaults synchronize];
    ...
    [self.sharedDefaults setInteger:self.numberOfImages forKey:@"layoutC_count"];
    [self.sharedDefaults setFloat:self.cropSize.width forKey:@"layoutC_cropSizeW"];
    [self.sharedDefaults setFloat:self.cropSize.height forKey:@"layoutC_cropSizeH"];
    
    [self.sharedDefaults setBool:true forKey:@"layoutC_isReady"];
    [self.sharedDefaults synchronize];
...
-(void)createWidget {
    ...
    TodayViewControllerLayoutCustom3* widget = [[TodayViewControllerLayoutCustom3 alloc] init];
