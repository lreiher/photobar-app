//
//  TodayViewControllerLayoutCustom2.m
//  PhotoBar
//
...
@interface TodayViewControllerLayoutCustom2 ()
...
@implementation TodayViewControllerLayoutCustom2

-(void)viewDidLoad {
    
    self.sharedDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.lennart.reiher.PhotoBar"];
    
    self.imgRatio = [self.sharedDefaults floatForKey:@"layoutB_ratio"];
    self.imgCount = [self.sharedDefaults integerForKey:@"layoutB_count"];
...
-(UIView*)createNextViewPinningHorizontally:(UIView*)distanceHolder2 :(NSInteger)pImgCount {
    ...
    if (pImgCount > 1) {
        ...
        CGFloat imgRatio = [self.sharedDefaults floatForKey:@"layoutB_ratio"];
...
-(void)loadImages {
    ...
    NSString* pathComponent = @"layoutB_image";
...
-(CGFloat)getPreferredHeight {
    ...
    if ([self.sharedDefaults boolForKey:@"layoutB_isHorizontal"]) return self.view.frame.size.width / self.imgCount / self.imgRatio;
