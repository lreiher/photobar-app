...
#import "TodayViewControllerLayout3.h"
#import <Photos/Photos.h>

@implementation TodayViewControllerLayout3
...
-(void)loadImages {
    ...
    NSString* pathComponent = @"layout3_image";
...
-(void)loadPhotosFromDiskWithPathComponent:(NSString*)pathComponent {
    ...
    self.imageView6.contentMode = UIViewContentModeScaleToFill;
    self.imageView7.contentMode = UIViewContentModeScaleToFill;
    self.imageView8.contentMode = UIViewContentModeScaleToFill;
    ...
    filesWithSelectedPrefix = [files filteredArrayUsingPredicate:
                               [NSPredicate predicateWithFormat:
                                [NSString stringWithFormat:@"self BEGINSWITH[cd] '%@6'",pathComponent]]];
    
    randomIndex = arc4random_uniform((u_int32_t)filesWithSelectedPrefix.count / 2) + 1;
    
    url = [self.storeURL URLByAppendingPathComponent:[NSString stringWithFormat:@"%@6_%ld",pathComponent,(long)randomIndex]];
    ...
    filesWithSelectedPrefix = [files filteredArrayUsingPredicate:
                               [NSPredicate predicateWithFormat:
                                [NSString stringWithFormat:@"self BEGINSWITH[cd] '%@7'",pathComponent]]];
    
    randomIndex = arc4random_uniform((u_int32_t)filesWithSelectedPrefix.count / 2) + 1;
    
    url = [self.storeURL URLByAppendingPathComponent:[NSString stringWithFormat:@"%@7_%ld",pathComponent,(long)randomIndex]];
    ...
    filesWithSelectedPrefix = [files filteredArrayUsingPredicate:
                               [NSPredicate predicateWithFormat:
                                [NSString stringWithFormat:@"self BEGINSWITH[cd] '%@8'",pathComponent]]];
    
    randomIndex = arc4random_uniform((u_int32_t)filesWithSelectedPrefix.count / 2) + 1;
    
    url = [self.storeURL URLByAppendingPathComponent:[NSString stringWithFormat:@"%@8_%ld",pathComponent,(long)randomIndex]];
...
-(void)loadPhotosFromCameraRollAtScale:(CGFloat)pScale {
    ...
    if (fetchResult.count > 4) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:asset5
                                                   targetSize:CGSizeMake(self.imageView8.frame.size.width*pScale,self.imageView8.frame.size.height*pScale)
                                                  contentMode:PHImageContentModeAspectFill
                                                      options:requestOptions
                                                resultHandler:^(UIImage *result, NSDictionary *info) {
                                                    ...
                                                    dispatch_async(dispatch_get_main_queue(), ^{
                                                        ...
                                                        self.imageView8.image = result;
                                                        
                                                    });
                                                }];
    }
    if (fetchResult.count > 5) {
        PHAsset* asset6 = [fetchResult objectAtIndex:fetchResult.count-6];
        [[PHImageManager defaultManager] requestImageForAsset:asset6
                                                   targetSize:CGSizeMake(self.imageView7.frame.size.width*pScale,self.imageView7.frame.size.height*pScale)
                                                  contentMode:PHImageContentModeAspectFill
                                                      options:requestOptions
                                                resultHandler:^(UIImage *result, NSDictionary *info) {
                                                    ...
                                                    dispatch_async(dispatch_get_main_queue(), ^{
                                                        ...
                                                        self.imageView7.image = result;
                                                        
                                                    });
                                                }];
    }
    if (fetchResult.count > 6) {
        PHAsset* asset7 = [fetchResult objectAtIndex:fetchResult.count-7];
        [[PHImageManager defaultManager] requestImageForAsset:asset7
                                                   targetSize:CGSizeMake(self.imageView6.frame.size.width*pScale,self.imageView6.frame.size.height*pScale)
                                                  contentMode:PHImageContentModeAspectFill
                                                      options:requestOptions
                                                resultHandler:^(UIImage *result, NSDictionary *info) {
                                                    ...
                                                    dispatch_async(dispatch_get_main_queue(), ^{
                                                        ...
                                                        self.imageView6.image = result;
                                                        
                                                    });
                                                }];
    }
    if (fetchResult.count > 7) {
        PHAsset* asset8 = [fetchResult objectAtIndex:fetchResult.count-8];
        [[PHImageManager defaultManager] requestImageForAsset:asset8
