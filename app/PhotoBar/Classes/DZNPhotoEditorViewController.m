...
- (instancetype)init
{
    self = [super init];
    if (self) {
        NSAssert(self.editingImage, @"Expecting an image or metadata for using the editor. Instead, use other initializer initWithImage:cropMode:cropSize:");
    }
    return nil;
}
...
- (CGRect)guideRect
{
    ...
    CGFloat marginX = (self.navigationController.view.bounds.size.width-_cropSize.width)/2;
    return CGRectMake(marginX, margin, _cropSize.width, _cropSize.height);
}
...
- (UIImage *)squareOverlayMask
{
    ...
    CGFloat marginX = (bounds.size.width-_cropSize.width)/2;
    ...
    if (height > width) {
        ...
        [clipPath moveToPoint:CGPointMake(marginX, height)];
        [clipPath addLineToPoint:CGPointMake(0, height)];
        [clipPath addLineToPoint:CGPointMake(0, 0)];
        [clipPath addLineToPoint:CGPointMake(marginX, 0)];
        [clipPath addLineToPoint:CGPointMake(marginX, height)];
        [clipPath closePath];
        [clipPath moveToPoint:CGPointMake(bounds.size.width, height)];
        [clipPath addLineToPoint:CGPointMake(marginX+width, height)];
        [clipPath addLineToPoint:CGPointMake(marginX+width,0)];
        [clipPath addLineToPoint:CGPointMake(bounds.size.width, 0)];
        [clipPath addLineToPoint:CGPointMake(bounds.size.width, height)];
        [clipPath closePath];
    ...
    // Add the square crop
    CGRect rect = CGRectMake(marginX+lineWidth/2, margin+lineWidth/2, width-lineWidth, height-lineWidth);
...
- (UIImage *)editedPhoto
{
    ...
    CGFloat horizontalMargin = (viewRect.size.width-guideRect.size.width)/2;

    guideRect.origin.x = -_scrollView.contentOffset.x - horizontalMargin;
...
- (void)cancelEdition:(id)sender
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:DZNPhotoPickerDidCancelPickingNotification object:nil];
}
