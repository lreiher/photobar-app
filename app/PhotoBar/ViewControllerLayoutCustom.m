...
#import "TodayViewControllerLayoutCustom.h"
#import "HeaderViewController.h"

@interface ViewControllerLayoutCustom ()
...
@implementation ViewControllerLayoutCustom
...
-(void)loadView {
    ...
    [super loadView];
    self.pathComponent = @"layoutA_image";
}
...
-(void)setInitialValues {
    ...
    self.defaultsKey = @"layoutA_needsUpdate";
    ...
    [self.sharedDefaults setBool:true forKey:@"layoutA_willAutoLayout"];
...
- (void)viewDidLayoutSubviews {
    if ([self.sharedDefaults boolForKey:@"layoutA_willAutoLayout"]) {
        [self didAutolayout];
    }
    if ([self.sharedDefaults integerForKey:@"layoutA_height"]) {
        self.heightSlider.value = [self.sharedDefaults integerForKey:@"layoutA_height"];
    }
...
-(void)didAutolayout {
    ...
    if ([self.sharedDefaults boolForKey:@"layoutA_isReady"]) {
        
        self.heightSlider.hidden = YES;
        
        self.cropSize = CGSizeMake([self.sharedDefaults floatForKey:@"layoutA_cropSizeW"], [self.sharedDefaults floatForKey:@"layoutA_cropSizeH"]);
        self.numberOfImages = [self.sharedDefaults integerForKey:@"layoutA_count"];
        ...
        [self.sharedDefaults setBool:(self.segmentedControl.selectedSegmentIndex == 0) forKey:@"layoutA_isHorizontal"];
        [self.sharedDefaults synchronize];
    }
    
    [self determinePlaceholderVisibility];
    
    [self.sharedDefaults setBool:false forKey:@"layoutA_willAutoLayout"];
    [self.sharedDefaults synchronize];
}
...
-(void)resetLayout {
    ...
    [self.sharedDefaults setBool:false forKey:@"layoutA_isReady"];
...
- (IBAction)confirmedSize:(id)sender {
    ...
    if (self.imageNoSlider.hidden && sender != nil) {    //confirmed size
        ...
        [self.sharedDefaults setInteger:self.heightSlider.value forKey:@"layoutA_height"];
        [self.sharedDefaults synchronize];
...
-(void)setAdditionalValues {
    ...
    CGFloat imgRatio;
    if ([self.sharedDefaults boolForKey:@"layoutA_isHorizontal"]) {
        imgRatio = (self.view.frame.size.width / (float)self.numberOfImages / height);
    } else {
        imgRatio = (self.view.frame.size.width * (float)self.numberOfImages / height);
    }
    [self.sharedDefaults setFloat:imgRatio forKey:@"layoutA_ratio"];
    [self.sharedDefaults synchronize];
    ...
    [self.sharedDefaults setInteger:self.numberOfImages forKey:@"layoutA_count"];
    [self.sharedDefaults setFloat:self.cropSize.width forKey:@"layoutA_cropSizeW"];
    [self.sharedDefaults setFloat:self.cropSize.height forKey:@"layoutA_cropSizeH"];
    
    [self.sharedDefaults setBool:true forKey:@"layoutA_isReady"];
    [self.sharedDefaults synchronize];
...
-(void)createWidget {
    ...
    TodayViewControllerLayoutCustom* widget = [[TodayViewControllerLayoutCustom alloc] init];
...
- (IBAction)segmentTriggered:(id)sender {
    ...
    CGFloat minImgWidth;
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) minImgWidth = 140.0;
    else minImgWidth = 60.0;
    
    if (![self.sharedDefaults boolForKey:@"layoutA_isHorizontal"]) {
        self.imageNoSlider.minimumValue = 2;
        self.imageNoSlider.maximumValue = ((NSInteger)(self.heightSlider.value / minImgWidth) > 2) ? (NSInteger)(self.heightSlider.value / minImgWidth) : 2;
    } else {
        self.imageNoSlider.minimumValue = 1;
        self.imageNoSlider.maximumValue = (NSInteger)(self.view.frame.size.width / minImgWidth);
    }
