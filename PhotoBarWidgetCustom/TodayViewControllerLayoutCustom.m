//
//  TodayViewControllerLayoutCustom.m
//  PhotoBar
//
//  Created by Lennart Reiher on 16.12.14.
...
@interface TodayViewControllerLayoutCustom ()
...
@implementation TodayViewControllerLayoutCustom

-(void)viewDidLoad {
    
    self.sharedDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.lennart.reiher.PhotoBar"];
    
    self.imgRatio = [self.sharedDefaults floatForKey:@"layoutA_ratio"];
    self.imgCount = [self.sharedDefaults integerForKey:@"layoutA_count"];
...
-(UIView*)createNextViewPinningHorizontally:(UIView*)distanceHolder2 :(NSInteger)pImgCount {
    ...
    if (pImgCount > 1) {
        ...
        CGFloat imgRatio = [self.sharedDefaults floatForKey:@"layoutA_ratio"];
...
-(void)loadImages {
    ...
    NSString* pathComponent = @"layoutA_image";
...
-(CGFloat)getPreferredHeight {
    ...
    if ([self.sharedDefaults boolForKey:@"layoutA_isHorizontal"]) return self.view.frame.size.width / self.imgCount / self.imgRatio;
